  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-center gap-4 mb-6">
        <button onClick={() => setTab('checkin')} className={`px-4 py-2 rounded-md font-semibold transition-colors ${tab === 'checkin' ? 'bg-blue-600 text-white shadow' : 'bg-gray-200 hover:bg-gray-300'}`}>Check-In</button>
        <button onClick={() => setTab('history')} className={`px-4 py-2 rounded-md font-semibold transition-colors ${tab === 'history' ? 'bg-blue-600 text-white shadow' : 'bg-gray-200 hover:bg-gray-300'}`}>History</button>
      </div>

      {tab === 'checkin' ? (
        <form onSubmit={handleCheckIn} className="bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Visitor Name" value={form.visitorName} onChange={(e) => setForm({...form, visitorName: e.target.value})} className="w-full p-3 pl-10 border rounded-lg bg-gray-50 border-gray-300" required />
            </div>
            <div className="relative">
              ...
              <input type="text" placeholder="Phone" value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className="w-full p-3 pl-10 border rounded-lg bg-gray-50 border-gray-300" required />
            </div>
            <div className="relative">
              ...
              <input type="text" placeholder="Person To Meet" value={form.personToMeet} onChange={(e) => setForm({...form, personToMeet: e.target.value})} className="w-full p-3 pl-10 border rounded-lg bg-gray-50 border-gray-300" required />
            </div>
            <div className="relative">
              ...
              <input type="text" placeholder="Purpose" value={form.purpose} onChange={(e) => setForm({...form, purpose: e.target.value})} className="w-full p-3 pl-10 border rounded-lg bg-gray-50 border-gray-300" required />
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {visitors.map(v => (
                <tr key={v._id} className="border-t border-gray-200 hover:bg-gray-50 transition-colors">
